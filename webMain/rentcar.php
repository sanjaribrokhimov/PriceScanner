                                ...
                                <ul class="list-unstyled mb-0 categories-list">
                                    ...
                                    <li>
                                        <div class="form-group filter">
                                            <input id="seats" type="text" class="form-control" placeholder="seats">
                                        </div>
                                    </li>
                                    <li>
                                        <div>
                                    ...
                                    <li>
                                        <div>
                                            <select id="fuel_type" class="form-select" name="fuel_type">
                                                <option value="">Yo'lovchi turini tanlang</option>
                                    ...
                                    <li>
                                        <div class="form-group filter">
                                            <select id="transmission" class="form-select" name="transmission">
                                                <option value="">Transmissiya turini tanlang</option>
                                    ...
                                    <li>
                                        <div class="form-group filter">
                                            <select id="climate" class="form-select" name="climate">
                                                <option value="">Iqlim turini tanlang</option>
                                                <option value="air_conditioning">Havo konditsioneri</option>
                                                <option value="climate_control">Iqlim nazorati</option>
                                            </select>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="form-group filter">
                                            <select id="insurance" class="form-select" name="insurance">
                                                <option value="">Sug'urta turini tanlang</option>
...
function cannotRender() {
    ...
    productsContainer.innerHTML = `
            <p style="width: 100%;text-align: center;font-size:20px">Soʻrovlaringiz asosida hech qanday mashina topilmadi.!</p>
        `
...
async function firstFilterData(params = '', isFirst = false) {
    ...
    const url =
        `${local_url}/api/rentcar/companies/web_main/cars-filtered?per_page=${filterState.per_page}&page=${filterState.page}`;
    try {
...
async function filterData(pageNumber = 1) {
    ...
    filterState.color = document.getElementById('color').value;
    filterState.seats = document.getElementById("seats").value;
    filterState.fuel_type = document.getElementById("fuel_type").value;
    filterState.transmission = document.getElementById("transmission").value;
    filterState.climate = document.getElementById("climate").value;
    filterState.page = pageNumber; // Обновляем текущую страницу
    ...
    const url = `${local_url}/api/rentcar/companies/web_main/cars-filtered?${params.toString()}`;
...
const filterInputs = [
    'min_price', 'max_price', 'color', 'seats', 'fuel_type', 'transmission', 'climate', 'insurance',
];
...
function renderData(data) {
    if (data.total === 0) cannotRender()
    else {
        const productsContainer = document.getElementById("products-container");
        productsContainer.innerHTML = ""; // Очищаем предыдущие результаты

        // Отрисовка продуктов
        data.cars.forEach(product => {
            const productHTML = `
                    <div class="col">
                        <div class="card rounded-0 product-card">
                            <img src="data:image/png;base64,${product.image}" class="card-img-top" alt="${product.model}">
                            <div class="card-body">
                                ...
                                <div class="product-action mt-2">
                                    <div class="d-grid gap-2">
                                        <a href="javascript:;" class="btn btn-light btn-ecomm" onclick="toCart(\`${product.id}\`, 'car')" data-bs-toggle="modal" data-bs-target="#QuickViewProduct">Savatchaga qo'shish</a>
                                        <a href="javascript:;" class="btn btn-link btn-ecomm" data-bs-toggle="modal" data-bs-target="#QuickViewProduct" onclick="showCarDetails(\`${product.model}\`, \`${product.price}\`, \`${product.company.name}\`, \`${product.image}\`, \`${product.color}\`, \`${product.year}\`, \`${product.seats}\`, \`${product.fuel_type}\`, \`${product.transmission}\`, \`${product.deposit}\`, \`${product.insurance}\`, \`${product.comment}\`, \`${product.id}\`)">
...
function changePage(pageNumber) {
    filterData(pageNumber); // Загружаем данные для выбранной страницы
}
</script>
